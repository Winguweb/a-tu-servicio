  ...
  new Vue({
    ...
    methods: {
      ...
      parseStep: function(step){
        ...
        step.forEach(function(answer) {
          var currentValue = answer.answer_data.value

          answers[currentValue] = {
            value: answers[currentValue] ? answers[currentValue].value + 1 : 1,
            percentage: answers[currentValue] ? (answers[currentValue].value + 1) / total * 100 : 1 / total * 100,
            key: currentValue,
            label: answer.answer_data.label
          }
        })
        return answers
      },
